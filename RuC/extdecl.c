...
modeeq(ruc_context *context, int first_mode, int second_mode)
{
    ...
    n = mode == MSTRUCT || mode == MFUNCTION
        ? 2 + context->modetab[first_mode + 2]
        : 1;

    for (i = 1; i <= n && flag; i++)
        flag = context->modetab[first_mode + i] ==
            context->modetab[second_mode + i];
...
totreef(ruc_context *context, int op)
{
    ...
    if (context->ansttype == LFLOAT &&
        ((op >= ASS && op <= DIVASS) || (op >= ASSAT && op <= DIVASSAT) ||
         (op >= EQEQ && op <= UNMINUS)))
...
getstatic(ruc_context *context, int type)
{
    ...
    context->displ += context->lg *
        szof(context, type); // lg - смещение от l (+1) или от g (-1)
    if (context->lg > 0)
...
{ // f = -1, если ф-ция-параметр, f>=1000, если это описание типа
  // f = -2, #define
    ...
    if (f != 1 &&
        ...
        if (context->func_def == 3 ? 1
                                   : context->identab[pred + 1] > 0
                    ? 1
                    : context->func_def == 1 ? 0 : 1)
            error(context,
    ...
    if (f == -2) // #define
    {
        ...
        context->identab[context->id + 3] =
            type; // это целое число, определенное по #define
    }
    else
    {
        // дальше тип или ссылка на modetab (для функций и структур)
        context->identab[context->id + 2] =
            type; // тип -1 int, -2 char, -3 float, -4 long, -5 double,
                  // если тип > 0, то это ссылка на modetab
        if (f == 1)
        {
            context->identab[context->id + 2] =
                0; // 0, если первым встретился goto, когда встретим метку,
                   // поставим 1
            context->identab[context->id + 3] =
                0; // при генерации кода когда встретим метку, поставим pc
        }
        else if (f >= 1000)
            context->identab[context->id + 3] =
                f; // это описание типа, если f > 1000, то f-1000 - это номер
                   // иниц проц
        else if (f)
        {
            if (f < 0)
...
binop(ruc_context *context, int sp)
{
    ...
    if ((op == LOGOR || op == LOGAND || op == LOR || op == LEXOR ||
         op == LAND || op == LSHL || op == LSHR || op == LREM) &&
                            ...
                            // передать параметром
{
    ...
    {
        if (context->anst == IDENT)
            context->tree[context->tc - 2] =
                is_float(context, context->ansttype) ? TIdenttovald
                                                     : TIdenttoval;

        if (!(is_array(context, context->ansttype) ||
              is_pointer(context, context->ansttype)))
            if (context->anst == ADDR)
                totree(context,
                       is_float(context, context->ansttype) ? TAddrtovald
                                                            : TAddrtoval);
        context->anst = VAL;
    }
}
...
mustbepointstring(ruc_context *context)
{
    ...
    if (!(context->ansttype > 0 &&
          ...
          context->modetab[context->modetab[context->ansttype + 1] + 1] ==
              LCHAR))
...
mustberowofint(ruc_context *context)
{
    ...
    if (!(context->ansttype > 0 &&
          context->modetab[context->ansttype] == MARRAY &&
          (context->modetab[context->ansttype + 1] == LINT ||
           context->modetab[context->ansttype + 1] == LCHAR)))
...
primaryexpr(ruc_context *context)
{
    ...
    {
        if (context->ansttype ==
            LFLOAT) // context->ansttype задается прямо в сканере
        {
    ...
    else if (context->cur == STRING)
    {
        int i;
        context->ansttype =
            newdecl(context, MARRAY, LCHAR); // теперь пишем context->ansttype в
        ...
        context->stackoperands[++context->sopnd] =
            context->ansttype; // ROWOFCHAR
        context->anst = VAL;
    }
    else if (context->cur == IDENT)
    {
         ...
         */
        {
            ...
            totree(context,
                   context->anstdispl = context->identab[context->lastid + 3]);
            context->stackoperands[++context->sopnd] = context->ansttype =
                context->identab[context->lastid + 2];
            context->anst = IDENT;
        }
    }
    ...
    else if (context->cur <= STANDARD_FUNC_START) // стандартная функция
    {
        ...
        { // процедуры управления параллельными нитями
            if (func == TINIT || func == TDESTROY || func == TEXIT)
                ; // void()
            else if (func == TMSGRECEIVE ||
                     func == TGETNUM) // getnum int()   msgreceive msg_info()
            {
                context->anst = VAL;
                context->ansttype = context->stackoperands[++context->sopnd] =
                    func == TGETNUM
                    ? LINT
                    : 2; // 2 - это ссылка на msg_info
            ...
            { // MSGSEND void(msg_info)  CREATE int(void*(*func)(void*))
              // SEMCREATE int(int)  JOIN,  SLEEP,  SEMWAIT,  SEMPOST void(int)
                ...
                if (func == TCREATE)
                {
                    ...
                    applid(context);
                    if (context->identab[context->lastid + 2] !=
                        15) // 15 - это аргумент типа void* (void*)
                ...
                else
                {
                    ...
                    {
                        if (context->ansttype !=
                            2) // 2 - это аргумент типа msg_info (struct{int
                               // numTh; int data;})
           ...
           int          stype) // выдает смещение до найденного поля или ошибку
{
    ...
    for (i = 0; i < context->modetab[stype + 2];
...
postexpr(ruc_context *context)
{
    ...
    if (context->next == LEFTBR) // вызов функции
    {
        ...
        n = context->modetab[leftansttyp +
                             2]; // берем количество аргументов функции
        ...
        {
            int mdj = context->leftansttype =
                context->modetab[j]; // это вид формального параметра, в
            ...
            else
            {
                ...
                else
                {
                    ...
                    if (is_int(context, mdj) &&
                        is_float(context, context->ansttype))
                        error(context, float_instead_int);

                    if (is_float(context, mdj) &&
                        is_int(context, context->ansttype))
                        insertwiden(context);
                    --context->sopnd;
                }
            }
        ...
        totree(context, lid);
        context->stackoperands[context->sopnd] = context->ansttype =
            context->modetab[leftansttyp + 1];
        context->anst = VAL;
        if (is_struct(context, context->ansttype))
            context->x -= context->modetab[context->ansttype + 1] - 1;
    }

    while (context->next == LEFTSQBR || context->next == ARROW ||
           context->next == DOT)
    {
        while (context->next ==
               LEFTSQBR) // вырезка из массива (возможно, многомерного)
        {
            int elem_type;
            if (was_func)
                error(context, slice_from_func);
            if (context->modetab[context->ansttype] !=
                MARRAY) // вырезка не из массива
        ...
        while (context->next ==
               ARROW) // это выборка поля из указателя на структуру, если после
        ...
        { // перед выборкой мог быть вызов функции или вырезка элемента массива
            ...
            if (context->modetab[context->ansttype] != MPOINT ||
                context->modetab[context->modetab[context->ansttype + 1]] !=
                    MSTRUCT)
            ...
            context->anstdispl = find_field(
                context,
                context->ansttype = context->modetab[context->ansttype + 1]);
            selectend(context);
        }
        if (context->next == DOT)
        ...
        {
            if (context->ansttype < 0 ||
                context->modetab[context->ansttype] != MSTRUCT)
            ...
            if (context->anst == VAL) // структура - значение функции
            {
                ...
                while (context->next == DOT)
                    context->anstdispl +=
                        find_field(context, context->ansttype);
            ...
            else if (context->anst == IDENT)
            {
                ...
                while (context->next == DOT)
                    context->anstdispl +=
                        globid * find_field(context, context->ansttype);
                context->tree[context->tc - 1] = context->anstdispl;
            }
            else // ADDR
            {
...
unarexpr(ruc_context *context)
{
    ...
    if (context->cur == LNOT || context->cur == LOGNOT ||
        context->cur == LPLUS || context->cur == LMINUS ||
        context->cur == LAND || context->cur == LMULT || context->cur == INC ||
        context->cur == DEC)
    {
        ...
        else
        {
            ...
            else if (op == LMULT)
            {
                ...
                context->stackoperands[context->sopnd] = context->ansttype =
                    context->modetab[context->ansttype + 1];
                context->anst = ADDR;
            }
            else
            {
...
inition(ruc_context *context, int decl_type)
{
    if (decl_type < 0 ||
        ...
        (is_array(context, decl_type) &&
         context->modetab[decl_type + 1] == LCHAR)) // или строк
    {
        exprassn(context, 1);
        toval(context);
        totree(context, TExprend);
            ...
            int decl_type) // сейчас context->modetab[decl_type] равен MSTRUCT
{
    ...
    for (i = 0; i < nf; i++)
    {
        ...
        {
            if (context->next ==
                COMMA) // поля инициализации идут через запятую, заканчиваются }
                scaner(context);
            else
...
exprassn(ruc_context *context, int level)
{
    ...
    if (opassn(context))
    {
        ...
        if (is_struct(context, ltype)) // присваивание в структуру
        {
            ...
            {
                opp = leftanst == IDENT
                    ? context->anst == IDENT ? COPY00 : COPY01
        ...
        {
            if (is_pointer(context, ltype) &&
                opp != ASS) // в указатель можно присваивать только с помощью =
                error(context, wrong_struct_ass);
            ...
            if (is_pointer(context, ltype) && is_pointer(context, rtype) &&
                ltype != rtype) // проверка нужна только для указателей
                error(context, type_missmatch);
                                    ...
                                    // структур из массивов сразу после idorpnt
{
    ...
    while (context->next ==
           LEFTSQBR) // это определение массива (может быть многомерным)
    {
        context->arrdim++;
        scaner(context);
        if (context->next == RIGHTSQBR)
        {
            scaner(context);
            if (context->next ==
                LEFTSQBR) // int a[][]={{1,2,3}, {4,5,6}} - нельзя;
                error(context,
...
{ // если встретятся массивы (прямо или в структурах), их размеры уже будут в
  // стеке
    int oldid = toidentab(context, 0, decl_type), elem_type, all,
        adN; // all - место в дереве, где будет общее количество выражений в
             // инициализации, для массивов - только признак (1) наличия
             // инициализации
    context->usual = 1;
    context->arrdim =
        0; // arrdim - размерность (0-скаляр), д.б. столько выражений-границ
    elem_type = decl_type;
        ...
        LEFTSQBR) // это определение массива (может быть многомерным)
    {
        ...
        adN = context->tc++;
        decl_type = context->identab[oldid + 2] = arrdef(
            context, decl_type); // Меняем тип (увеличиваем размерность массива)
    ...
    context->tree[context->tc++] = is_pointer(context, decl_type)
        ? 0
        : context->was_struct_with_arr; // proc
    ...
    if (context->next == ASS)
    {
        ...
        if (is_array(context, decl_type)) // инициализация массива
        {
            ...
            if (context->onlystrings == 1)
                context->tree[all + 2] = context->usual +
                    2; // только из строк 2 - без границ, 3 - с границами
        }
        else
        {
            inition(context, decl_type);
...
statement(ruc_context *context)
{
    ...
    scaner(context);
    if ((is_int(context, context->cur) || is_float(context, context->cur) ||
         context->cur == LVOID || context->cur == LSTRUCT) &&
        context->blockflag)
    ...
    else if (context->cur == IDENT && context->next == COLON)
    {
        ...
        if (flag)
        {
            ...
            context->gotost[context->pgotost++] =
                id; // это определение метки, если она встретилась до переходов
                    // на нее
    ...
    else
    {
        ...
        switch (context->cur)
        {
            ...
            case PRINTF:
            {
                ...
                paramnum =
                    evaluate_params(context, fnum = context->num, formatstr,
                                    formattypes, placeholders);
...
struct_decl_list(ruc_context *context)
{
    ...
    do
    {
        t = elem_type = idorpnt(context, wait_ident_after_semicomma_in_struct,
                                gettype(context));
        if (context->next == LEFTSQBR)
        {
            ...
            if (context->next == ASS)
            {
                ...
                if (is_array(context, t)) // инициализация массива
                {
                    ...
                    if (context->onlystrings == 1)
                        context->tree[all + 2] =
                            context->usual + 2; // только из строк 2 - без
...
*/              }
    ...
    if (wasarr)
    {
        ...
        context->tree[tstrbeg + 1] = context->was_struct_with_arr =
            context->procd++;
    }
    else
    {
...
gettype(ruc_context *context)
{
    ...
    if (is_int(context, context->type = context->cur) ||
        is_float(context, context->type) || context->type == LVOID)
        return (context->cur == LLONG
                    ? LINT
                    : context->cur == LDOUBLE ? LFLOAT : context->type);
    ...
    else if (context->cur == IDENT)
    {
        ...
        context->was_struct_with_arr =
            context->identab[context->lastid + 3] - 1000;
...

{
    ...
    while (is_int(context, context->next) || is_float(context, context->next) ||
           context->next == LSTRUCT || context->next == LVOID)
    {
        int repeat = 1;
        scaner(context);
        ...
        {
            decl_id(context,
                    idorpnt(context, after_type_must_be_ident, firstdecl));
...
func_declarator(ruc_context *context, int level, int func_d, int firstdecl)
{
    ...
    int loc_modetab[100], locmd, numpar = 0, ident, maybe_fun, repeat = 1, i,
                                 wastype = 0, old;
    ...
    while (repeat)
    {
        if (context->cur == LVOID || is_int(context, context->cur) ||
            is_float(context, context->cur) || context->cur == LSTRUCT)
        {
            ...
            if (context->next == LMULT)
            {
                maybe_fun = 1;
                scaner(context);
                context->type = context->type == LVOID
                    ? LVOIDASTER
                    : newdecl(context, MPOINT, context->type);
            }
            if (level)
            {
        ...
        if (wastype)
        {
            ...
            if (context->next == LEFTBR)
            {
                ...
                loc_modetab[locmd - 1] =
                    func_declarator(context, 0, 2, context->type);
...
ext_decl(ruc_context *context)
{
    ...
    do // top level описания переменных и функций до конца файла
    {
        ...
        context->func_def =
            3; // context->func_def = 0 - (), 1 - определение функции, 2 - это
           ...
           // только одно, никаких ','
        {
            ...
            {
                scaner(context);
                context->type = context->firstdecl == LVOID
                    ? LVOIDASTER
                    : newdecl(context, MPOINT, context->firstdecl);
            }
