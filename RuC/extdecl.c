...
int modeeq(ruc_context *context, int first_mode, int second_mode)
{
    int n, i, flag = 1, mode;
...
int check_duplicates(ruc_context *context)
{
...
int newdecl(ruc_context *context, int type, int elemtype)
{
...
int evaluate_params(ruc_context *context, int num, int formatstr[], int formattypes[], int placeholders[])
{
    int numofparams = 0;
    int i = 0, fsi;
//           for (i=0; i<num; i++)
...
int szof(ruc_context *context, int type)
{
    return context->next == LEFTSQBR ? 1 : type == LFLOAT ? 2 :
    (type > 0 && context->modetab[type] == MSTRUCT) ? context->modetab[type + 1] : 1;
}

int is_row_of_char(ruc_context *context, int t)
{
    return t > 0 && context->modetab[t] == MARRAY && context->modetab[t + 1] == LCHAR;
}

int is_function(ruc_context *context, int t)
{
    return t > 0 && context->modetab[t] == MFUNCTION;
}

int is_array(ruc_context *context, int t)
{
    return t > 0 && context->modetab[t] == MARRAY;
}

int is_pointer(ruc_context *context, int t)
{
    return t > 0 && context->modetab[t] == MPOINT;
}

int is_struct(ruc_context *context, int t)
{
    return t > 0 && context->modetab[t] == MSTRUCT;
}

int is_float(ruc_context *context, int t)
{
    UNUSED(context);
    return t == LFLOAT || t == LDOUBLE;
}

int is_int(ruc_context *context, int t)
{
    UNUSED(context);
    return t == LINT || t == LLONG || t == LCHAR;
}

void mustbe(ruc_context *context, int what, int e)
{
    if (scaner(context) != what)
        error(context, e);
}

void totree(ruc_context *context, int op)
{
    context->tree[context->tc++] = op;
}

void totreef(ruc_context *context, int op)
{
    context->tree[context->tc++] = op;
...
int getstatic(ruc_context *context, int type)
{
    int olddispl = context->displ;
    context->displ += context->lg * szof(context, type);            // lg - смещение от l (+1) или от g (-1)
    if (context->lg > 0)
        context->maxdispl = (context->displ > context->maxdispl)
            ? context->displ : context->maxdispl;
    else
        context->maxdisplg = -context->displ;
    return olddispl;
}

int toidentab(ruc_context *context, int f, int type)       // f =  0, если не ф-ция, f=1, если метка, f=funcnum, если описание ф-ции,
{                                    // f = -1, если ф-ция-параметр, f>=1000, если это описание типа
                                     // f = -2, #define
//    printf("\n f= %i context->repr %i rtab[context->repr] %i rtab[context->repr+1] %i rtab[context->repr+2] %i\n", f, context->repr, context->reprtab[context->repr], context->reprtab[context->repr+1], context->reprtab[context->repr+2]);
    int pred;
    ...
    pred = context->identab[context->id] = context->reprtab[context->repr + 1]; // ссылка на описание с таким же представлением в предыдущем блоке
    if (pred)                               // pred == 0 только для main, эту ссылку портить нельзя
        context->reprtab[context->repr + 1] = context->id;             // ссылка на текущее описание с этим представлением (это в reprtab)
    ...
    if (f != 1 && pred >= context->curid)            // один  и тот же идент м.б. переменной и меткой
        ...
        if (context->func_def == 3 ? 1 : context->identab[pred + 1] > 0 ? 1 : context->func_def == 1 ? 0 : 1)
            error(context, repeated_decl);  // только определение функции может иметь 2 описания, т.е. иметь предописание

    context->identab[context->id + 1] = context->repr;                 // ссылка на представление
    if (f == -2)                            // #define
    {
    ...
    else
    {
        ...
        if (f == 1)
        {
            context->identab[context->id + 2] = 0;             // 0, если первым встретился goto, когда встретим метку, поставим 1
            context->identab[context->id + 3] = 0;             // при генерации кода когда встретим метку, поставим pc
        }
        else if (f >= 1000)
            context->identab[context->id + 3] = f;             // это описание типа, если f > 1000, то f-1000 - это номер иниц проц
        else if (f)
        {
            if (f < 0)
            {
            ...
            else                          // identtab[context->lastid+3] - номер функции, если < 0, то это функция-параметр
            {
...
void binop(ruc_context *context, int sp)
{
    ...
    //    printf("binop context->sopnd=%i ltype=%i rtype=%i context->ansttype=%i\n", context->sopnd, ltype, rtype, context->ansttype);
    context->anst = VAL;
}
...
void toval(ruc_context *context)        // надо значение положить на стек, например, чтобы передать параметром
{
    ...
    {
        if (context->anst == IDENT)
            context->tree[context->tc - 2] = is_float(context, context->ansttype)
            ? TIdenttovald : TIdenttoval;

        if (!(is_array(context, context->ansttype) || is_pointer(context, context->ansttype)))
            if (context->anst == ADDR)
                totree(context, is_float(context, context->ansttype)
                    ? TAddrtovald : TAddrtoval);
        context->anst = VAL;
    }
}

void insertwiden(ruc_context *context)
{
    context->tc--;
    totree(context, WIDEN);
...
void applid(ruc_context *context)
{
...
void actstring(ruc_context *context)
{
    int n = 0, adn;
...
/*        if (scaner(context) == IDENT)
        {
            ...
            if (context->identab[context->lastid+2] == 1)
                context->cur = NUMBER, context->ansttype = LINT, num = context->identab[context->lastid+3];
        }
 */
        if (scaner(context) == NUMBER &&
            (context->ansttype == LINT || context->ansttype == LCHAR))
...
void mustbestring(ruc_context *context)
{
    scaner(context);
    exprassn(context, 1);
    toval(context);
    context->sopnd--;
    if (! (context->ansttype > 0 &&
        context->modetab[context->ansttype] == MARRAY &&
        context->modetab[context->ansttype+1] == LCHAR) )
        error(context, not_string_in_stanfunc);
}

void mustbepointstring(ruc_context *context)
{
    scaner(context);
    exprassn(context, 1);
    toval(context);
    context->sopnd--;
    if (! (context->ansttype > 0 &&
           context->modetab[context->ansttype] == MPOINT &&
...
void mustbeint(ruc_context *context)
{
    scaner(context);
    exprassn(context, 1);
    toval(context);
...
void mustberowofint(ruc_context *context)
{
    scaner(context);
...
void primaryexpr(ruc_context *context)
{
    ...
    {
        int i;
        context->ansttype = newdecl(context, MARRAY, LCHAR); // теперь пишем context->ansttype в анализаторе, а не в сканере
        totree(context, TString);
        totree(context, context->num);
...
//            totree(context, CASTC);
    ...
    else if (context->cur <= STANDARD_FUNC_START)            // стандартная функция
    {
        ...
        {                                // процедуры управления параллельными нитями
            ...
            {
                context->anst = VAL;
                context->ansttype = context->stackoperands[++context->sopnd] = func == TGETNUM ? LINT : 2; // 2 - это ссылка на msg_info
            ...
            {               // MSGSEND void(msg_info)  CREATE int(void*(*func)(void*))
                ...
                else
                {
                    ...
                    else
                    {
                        ...
                        if (func == TSEMCREATE)
                            context->anst = VAL, context->ansttype = context->stackoperands[context->sopnd] = LINT; // съели 1 параметр, выдали int
                        else
                            --context->sopnd;                                // съели 1 параметр, не выдали результата
                    }
                }
            }
        }
...
void index_check(ruc_context *context)
{
    if (!is_int(context, context->ansttype))
        error(context, index_must_be_int);
}

int find_field(ruc_context *context, int stype)                          // выдает смещение до найденного поля или ошибку
{
...
void selectend(ruc_context *context)
{
...
void postexpr(ruc_context *context)
{
    int lid, leftansttyp;
    int was_func = 0;
    ...
    {
        int i, j, n, dn, oldinass = context->inass;
        was_func = 1;
        scaner(context);
        ...
        {
            int mdj = context->leftansttype = context->modetab[j];      // это вид формального параметра, в context->ansttype будет вид фактического параметра
            scaner(context);
            if (is_function(context, mdj)) // фактическим параметром должна быть функция, в С - это только идентификатор
            {
                if (context->cur != IDENT)
                    error(context, act_param_not_ident);
                applid(context);
    ...
    while (context->next == LEFTSQBR || context->next == ARROW || context->next == DOT)
    {
        ...
        {                      // перед выборкой мог быть вызов функции или вырезка элемента массива
            ...
            if (context->modetab[context->ansttype] != MPOINT || context->modetab[context->modetab[context->ansttype + 1]] != MSTRUCT)
            ...
            totree(context, TSelect);          // context->anst уже был ADDR, т.е. адрес теперь уже всегда на верхушке стека
...
void unarexpr(ruc_context *context)
{
    ...
    if (context->cur == LNOT || context->cur == LOGNOT || context->cur == LPLUS || context->cur == LMINUS ||
        context->cur == LAND || context->cur == LMULT ||
        context->cur == INC  || context->cur == DEC)
    {
        ...
        else
        {
            ...
            if (op == LAND)
            {
                ...
                context->stackoperands[context->sopnd] = context->ansttype = newdecl(context, MPOINT, context->ansttype);
                context->anst = VAL;
            }
            else if (op == LMULT)
            {
...
void exprinbrkts(ruc_context *context, int er)
{
...
void exprassninbrkts(ruc_context *context, int er)
{
    mustbe(context, LEFTBR, er);
    scaner(context);
    exprassnval(context);
...
void subexpr(ruc_context *context)
{
...
void inition(ruc_context *context, int decl_type)
{
...
void struct_init(ruc_context *context, int decl_type)   // сейчас context->modetab[decl_type] равен MSTRUCT
{
...
void exprassnvoid(ruc_context *context)
{
...
void exprassn(ruc_context *context, int level)
{
...
//        else if (is_array(context, context->leftansttype)) //пока в RuC присваивать массивы нельзя
//        array_init(context, context->leftansttype);
    ...
    if (opassn(context))
    {
        ...
        rtype = context->stackoperands[context->sopnd--];      // снимаем типы операндов со стека
        ...
        else // оба операнда базового типа или указатели
        {
            ...
            if (is_pointer(context, ltype) && is_pointer(context, rtype) && ltype != rtype)// проверка нужна только для указателей
                error(context, type_missmatch);
        ...
        condexpr(context);    // condexpr учитывает тот факт, что начало выражения в виде unarexpr уже выкушано
}
...
void array_init(ruc_context *context, int decl_type)                   // сейчас context->modetab[decl_type] равен MARRAY
{
    int ad, all = 0;
...
int arrdef(ruc_context *context, int t)                    // вызывается при описании массивов и структур из массивов сразу после idorpnt
{
    context->arrdim = 0;
    ...
    while (context->next == LEFTSQBR)          // это определение массива (может быть многомерным)
    {
        ...
        if (context->next == RIGHTSQBR)
        {
            ...
            if (context->next == LEFTSQBR)     // int a[][]={{1,2,3}, {4,5,6}} - нельзя;
                error(context, empty_init);    // границы определять по инициализации можно только по последнему изм.
            context->usual = 0;
        }
        else
        {
...
void decl_id(ruc_context *context, int decl_type)    // вызывается из block и extdecl, только эта процедура реально отводит память
{                              // если встретятся массивы (прямо или в структурах), их размеры уже будут в стеке
    ...
    if (context->next == LEFTSQBR)                                    // это определение массива (может быть многомерным)
    {
        ...
        adN = context->tc++;
        decl_type = context->identab[oldid + 2] = arrdef(context, decl_type);  // Меняем тип (увеличиваем размерность массива)
...
void statement(ruc_context *context)
{
    int flagsemicol = 1, oldwasdefault = context->wasdefault, oldinswitch = context->inswitch, oldinloop = context->inloop;
    context->wasdefault = 0;
    scaner(context);
    if ((is_int(context, context->cur) || is_float(context, context->cur) || context->cur == LVOID || context->cur == LSTRUCT) && context->blockflag)
    ...
    else if (context->cur == IDENT && context->next == COLON)
    {
        ...
        if (flag)
        {
            ...
            context->gotost[context->pgotost++] = id;              // это определение метки, если она встретилась до переходов на нее
...
int idorpnt(ruc_context *context, int e, int t)
{
...
int struct_decl_list(ruc_context *context)
{
...

{
    ...
    int notended = 1, i, olddispl, oldlg = context->lg, firstdecl;
    ...
    while (is_int(context, context->next) || is_float(context, context->next) || context->next == LSTRUCT || context->next == LVOID)
    {
        int repeat = 1;
        scaner(context);
...
void function_definition(ruc_context *context)
{
    int fn = context->identab[context->lastid + 3], i, pred, oldrepr = context->repr, ftype, n, fid = context->lastid;
    int olddispl = context->displ;
    context->pgotost = 0;
...
int func_declarator(ruc_context *context, int level, int func_d, int firstdecl)
{
    ...
    while (repeat)
    {
        if (context->cur == LVOID || is_int(context, context->cur) || is_float(context, context->cur) || context->cur == LSTRUCT)
        {
...
void ext_decl(ruc_context *context)
{
    int i;
 ...
 */
        ...
        context->func_def = 3;   // context->func_def = 0 - (), 1 - определение функции, 2 - это предописание, 3 - не знаем или вообще не функция
...
//          firstdecl = LINT;
        ...
        do            // описываемые объекты через ',' определение функции может быть только одно, никаких ','
        {
            ...
            if (context->next == LEFTBR)                // определение или предописание функции
            {
                ...
                scaner(context);
                scaner(context);
                context->type = func_declarator(context, first, 3, firsttype);  // выкушает все параметры до ) включительно
                if (context->next == BEGIN)
                {
                    ...
                    context->func_def = 2;
                // теперь я точно знаю, это определение ф-ции или предописание (context->func_def=1 или 2)
