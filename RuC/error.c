...
error(ruc_context *context, int ernum)
{
    ...
    // printer_printf(&context->err_options, "line - 1=%d, mline=%d, co[carnum-1=%d] = 1%c1, lines[line]=%d,
    // lines[line+1]=%d \n",
...
m_error(ruc_context *context, int ernum)
{
    ...
    switch (ernum)
    {
        case after_preproces_words_must_be_space:
            printer_printf(&context->err_options, "Неправильное ключевое слово препроцессора, далее должен "
                   "идти символ ' 'или'\\n'или'\\t' \n");
            break;
