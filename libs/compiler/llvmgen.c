...
static void emit_one_dimension_initialization(information *const info, const node *const nd, const item_t id,
                                              const item_t arr_type, const size_t cur_dimension,
                                              const item_t prev_slice, const bool is_local);
...
static void to_code_operation_reg_reg(information *const info, const binary_t operation, const size_t fst,
...
static void to_code_operation_reg_const_integer(information *const info, const binary_t operation, const size_t fst,
...
static void to_code_operation_reg_const_bool(information *const info, const binary_t operation, const size_t fst,
                                             const bool snd, const item_t type)
{
    uni_printf(info->sx->io, " %%.%zu = ", info->register_num);
    operation_to_io(info, operation, TYPE_INTEGER);
    uni_printf(info->sx->io, " ");
...
static void to_code_operation_reg_const_double(information *const info, const binary_t operation, const size_t fst,
                                               const double snd)
{
    uni_printf(info->sx->io, " %%.%zu = ", info->register_num);
...
static void to_code_operation_const_reg_integer(information *const info, const binary_t operation, const item_t fst,
                                                const size_t snd, const item_t type)
{
    uni_printf(info->sx->io, " %%.%zu = ", info->register_num);
    operation_to_io(info, operation, TYPE_INTEGER);
...
static void to_code_operation_const_reg_double(information *const info, const binary_t operation, const double fst,
                                               const size_t snd)
{
    uni_printf(info->sx->io, " %%.%zu = ", info->register_num);
    operation_to_io(info, operation, TYPE_FLOATING);
...
static void to_code_operation_reg_null(information *const info, const binary_t operation, const size_t fst,
                                       const item_t type)
{
    uni_printf(info->sx->io, " %%.%zu = ", info->register_num);
    operation_to_io(info, operation, TYPE_INTEGER);
...
static void to_code_operation_null_reg(information *const info, const binary_t operation, const size_t snd,
                                       const item_t type)
{
    uni_printf(info->sx->io, " %%.%zu = ", info->register_num);
    operation_to_io(info, operation, TYPE_INTEGER);
                              ...
                              const bool is_array, const bool is_pointer, const bool is_local)
{
    ...
    type_to_io(info, type);
    uni_printf(info->sx->io, " %s%s.%zu, ", /*ident_is_local(info->sx, reg)*/ true ? "%" : "@", is_pointer ? "var" : "",
               reg);
    type_to_io(info, type);
    uni_printf(info->sx->io, "* %s%s.%zu, align 4\n", is_local ? "%" : "@", is_array ? "" : "var", id);
}
...
static void to_code_alloc_array_dynamic(information *const info, const size_t index, const item_t type)
{
    ...
    for (size_t i = 2; i <= dim; i++)
    {
        uni_printf(info->sx->io, " %%.%zu = mul nuw i32 %%.%" PRIitem ", %%.%" PRIitem "\n", info->register_num,
                   to_alloc, hash_get_by_index(&info->arrays, index, i));
...
static void to_code_slice(information *const info, const item_t id, const size_t cur_dimension, const item_t prev_slice,
                          const item_t type, const bool is_local)
{
...
static void emit_identifier_expression(information *const info, const node *const nd)
{
    ...
    {
        to_code_load(info, info->register_num, is_addr_to_val ? info->register_num - 1 : id, type, is_addr_to_val,
                     is_addr_to_val || is_local);
        info->answer_reg = info->register_num++;
    }
    ...
    for (size_t i = 0; i < args; i++)
    {
        ...
        if (arguments_type[i] == ASTR)
        {
            ...
            uni_printf(info->sx->io,
                       ...
                       string_length + 1, string_length + 1, index);
...
static void emit_member_expression(information *const info, const node *const nd)
{
    ...
    uni_printf(info->sx->io,
               " %%.%zu = getelementptr inbounds %%struct_opt.%" PRIitem ", "
               "%%struct_opt.%" PRIitem "* %s.%zu, i32 0, i32 %" PRIitem "\n",
               info->register_num, type, type, is_complex ? "%" : (ident_is_local(info->sx, id) ? "%var" : "@var"),
               is_complex ? info->register_num - 1 : id, place);
...
static void emit_inc_dec_expression(information *const info, const node *const nd)
{
    ...
    switch (operation)
    {
        ...
        case UN_POSTDEC:
        {
            ...
            {
                to_code_operation_reg_const_integer(
                    info, operation == UN_PREINC || operation == UN_POSTINC ? BIN_ADD : BIN_SUB, info->register_num - 1,
                    1, operation_type);
            }
            else // double
            {
                to_code_operation_reg_const_double(
                    info, operation == UN_PREINC || operation == UN_POSTINC ? BIN_ADD : BIN_SUB, info->register_num - 1,
                    1.0);
            }
        }
        ...
        case UN_UPB:
        {
            ...
            if (!is_complex)
            {
                ...
                else
                {
                    ...
                    {
                        uni_printf(info->sx->io, " %%.%zu = mul nsw i32 %%.%zu, %%.%zu\n", info->register_num, upb_reg,
                                   (size_t)hash_get(&info->arrays, id, i));
...
static void emit_assignment_expression(information *const info, const node *const nd)
{
    ...
    if (assignment_type != BIN_ASSIGN)
    {
        ...
        {
            to_code_operation_reg_const_integer(info, assignment_type, info->register_num - 1, info->answer_const,
                                                operation_type);
        }
    ...
    {
        to_code_store_reg(info, result, id, operation_type, is_complex, info->answer_kind == AMEM,
                          !is_complex ? ident_is_local(info->sx, id) : true);
    ...
    {
        to_code_store_const_integer(info, info->answer_const, id, is_complex,
                                    !is_complex ? ident_is_local(info->sx, id) : true, operation_type);
    }
    else if (type_is_floating(info->sx, operation_type))
    {
        to_code_store_const_double(info, info->answer_const_double, id, is_complex,
                                   !is_complex ? ident_is_local(info->sx, id) : true);
    }
    else if (type_is_boolean(info->sx, operation_type))
    {
        to_code_store_const_bool(info, info->answer_const_bool, id, is_complex,
                                 !is_complex ? ident_is_local(info->sx, id) : true);
    }
}
        ...
        case BIN_LOG_AND:
        {
            ...
            if (!is_logic)
            {
                ...
                uni_printf(info->sx->io, " %%.%zu = phi i1 [ %s, %%%s%zu ], [ %%.%zu, %%label%zu ]\n",
                           info->register_num, operator== BIN_LOG_OR ? "true" : "false",
                           info->label_phi_previous == 0 ? "" : "label", info->label_phi_previous,
                           info->register_num - 1, label_next);
...
static void emit_initializer_expression(information *const info, const node *const nd)
{
    if (type_is_structure(info->sx, expression_get_type(nd)))
    {
        ...
        for (size_t i = 0; i < size; i++)
        {
            ...
            uni_printf(info->sx->io,
                       " %%.%zu = getelementptr inbounds %%struct_opt.%zu, "
                       "%%struct_opt.%zu* %%.%zu, i32 0, i32 %zu\n",
                       info->register_num, structure_type, structure_type, slice_reg, i);
                                              ...
                                              const item_t prev_slice, const bool is_local)
{
    ...
    for (size_t i = 0; i < size && size != SIZE_MAX; i++)
    {
        ...
        {
            emit_one_dimension_initialization(info, &initializer, id, arr_type, cur_dimension - 1, (item_t)slice_reg,
                                              true);
        }
    }
}
...
static void emit_initialization(information *const info, const node *const nd, const item_t id, const item_t arr_type)
{
        ...
        for (size_t i = 0; i < dimensions; i++)
        {
            hash_set_by_index(&info->arrays, index, 1 + i,
                              node_get_type(&list_expression) == OP_INITIALIZER
                                  ? (item_t)expression_initializer_get_size(&list_expression)
                                  : ITEM_MAX);
            list_expression = node_get_type(&list_expression) == OP_INITIALIZER
                                  ? expression_initializer_get_subexpr(&list_expression, 0)
                                  : node_broken();
        }
    ...
    // TODO: надо реализовать для большего количества измерений. Там сложность в том, что последнее измерение может
    // иметь различные границы массив инициализируется строкой
    ...
    else if (expression_get_class(nd) == EXPR_INITIALIZER && type_is_structure(info->sx, expression_get_type(nd)))
    {
        ...
        if (ident_is_local(info->sx, (size_t)id))
        {
            for (size_t i = 0; i < N && N != SIZE_MAX; i++)
            {
                ...
                uni_printf(info->sx->io,
                           " %%.%zu = getelementptr inbounds %%struct_opt.%" PRIitem ", %%struct_opt.%" PRIitem
                           "* %%var.%" PRIitem ", i32 0, i32 %zu\n",
...
static void emit_function_definition(information *const info, const node *const nd)
{
    ...
    const item_t ret_type =
        ref_ident != info->sx->ref_main ? type_function_get_return_type(info->sx, func_type) : TYPE_INTEGER;
...
static void emit_switch_statement(information *const info, const node *const nd)
{
    ...
    uni_printf(info->sx->io, " %%.%zu, label %%label%zu [\n", info->answer_reg,
               info->label_switch - case_num - has_default);
...
static void strings_declaration(information *const info)
{
    ...
    for (size_t i = 0; i < amount; i++)
    {
        ...
        uni_printf(info->sx->io, "@.str%zu = private unnamed_addr constant [%zu x i8] c\"", i, length + 1);
...
static void runtime(information *const info)
{
    ...
    uni_printf(info->sx->io, "@.str = private unnamed_addr constant [3 x i8] c\"%%s\\00\", align 1\n"
                             ...
                             " %%8 = call i32 (i8*, ...) @printf(i8* getelementptr inbounds ([3 x i8], [3 x i8]* "
                             "@.str, i32 0, i32 0), i8* %%7)\n"
