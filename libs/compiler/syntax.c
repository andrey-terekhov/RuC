...
static void repr_add_keyword(map *const reprtab, const char32_t *const eng, const char32_t *const rus,
                             const token_t token)
{
...
static inline bool type_is_equal(const syntax *const sx, const size_t first, const size_t second)
{
    ...
    if (type == TYPE_CONST)
    {
        return type_is_equal(sx, type_const_get_unqualified_type(sx, first),
                             type_const_get_unqualified_type(sx, second));
    }
...
size_t ident_add(syntax *const sx, const size_t repr, const item_t kind, const item_t type, const int func_def)
{
    ...
    if (kind != 1 && ident_get_prev(sx, last_id) != ITEM_MAX - 1 && prev >= sx->cur_id &&
        (func_def != 1 || ident_get_repr(sx, prev) > 0))
    {
...
bool type_is_boolean(const syntax *const sx, const item_t type)
{
    return type == TYPE_BOOLEAN ||
           (type_is_const(sx, type) && type_is_boolean(sx, type_const_get_unqualified_type(sx, type)));
}

bool type_is_integer(const syntax *const sx, const item_t type)
{
    return type_is_const(sx, type) ? type_is_integer(sx, type_const_get_unqualified_type(sx, type))
                                   : type == TYPE_CHARACTER || type == TYPE_INTEGER || type_is_enum(sx, type) ||
                                         type_is_enum_field(sx, type);
}

bool type_is_floating(const syntax *const sx, const item_t type)
{
    return type == TYPE_FLOATING ||
           (type_is_const(sx, type) && type_is_floating(sx, type_const_get_unqualified_type(sx, type)));
}
...
{
    return type_is_boolean(sx, type) || type_is_integer(sx, type) || type_is_pointer(sx, type) ||
           type_is_null_pointer(type);
}
...
bool type_is_file(const syntax *const sx, const item_t type)
{
    return type == TYPE_FILE ||
           (type_is_const(sx, type) && type_is_file(sx, type_const_get_unqualified_type(sx, type)));
}
...
item_t type_array_get_element_type(const syntax *const sx, const item_t type)
{
    return type_is_const(sx, type)   ? type_array_get_element_type(sx, type_const_get_unqualified_type(sx, type))
           : type_is_array(sx, type) ? type_get(sx, (size_t)type + 1)
                                     : ITEM_MAX;
}
...
size_t type_structure_get_member_amount(const syntax *const sx, const item_t type)
{
    return type_is_const(sx, type) ? type_structure_get_member_amount(sx, type_const_get_unqualified_type(sx, type))
           : type_is_structure(sx, type) ? (size_t)type_get(sx, (size_t)type + 2) / 2
                                         : SIZE_MAX;
}
...
size_t type_structure_get_member_name(const syntax *const sx, const item_t type, const size_t index)
{
    return type_is_const(sx, type)
               ? type_structure_get_member_name(sx, type_const_get_unqualified_type(sx, type), index)
           : type_is_structure(sx, type) ? (size_t)type_get(sx, (size_t)type + 4 + 2 * index)
                                         : SIZE_MAX;
}
...
{
    return type_is_const(sx, type)
               ? type_structure_get_member_type(sx, type_const_get_unqualified_type(sx, type), index)
           : type_is_structure(sx, type) ? type_get(sx, (size_t)type + 3 + 2 * index)
                                         : ITEM_MAX;
}
...
item_t type_pointer_get_element_type(const syntax *const sx, const item_t type)
{
    return type_is_const(sx, type)     ? type_pointer_get_element_type(sx, type_const_get_unqualified_type(sx, type))
           : type_is_pointer(sx, type) ? type_get(sx, (size_t)type + 1)
                                       : ITEM_MAX;
}
