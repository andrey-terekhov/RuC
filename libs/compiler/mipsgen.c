...
typedef enum LABEL
{
	...
	L_ERROR,		/**< Тип метки -- прыжок при ошибке */
...
/**
 * Checks if type is integer. That is: bool, char, int, enum, enum field
 * 
 * @param	sx					Syntax structure
 * @param	type				Type
 * 
 * @return	@c True if the type is integer, @c False otherwise
*/
static bool mips_type_is_integer(const syntax *const sx, const item_t type)
{
	return (type_is_boolean(type) || type_is_integer(sx, type));
}
	...
	if ((rval1->kind == RVALUE_KIND_REGISTER) && (rval2->kind == RVALUE_KIND_REGISTER))
	{
		...
		switch (operator)
		{
			...
			case BIN_NE:
			{
				...
				emit_conditional_branch(enc, instruction, &result_rvalue, &label_else);
	...
	else
	{
		...
		switch (operator)
		{
			...
			default:
			{
				...
				// TODO: вынести выше перед if-ом. Вообще, хочется в целом переделать эту функцию
...
static rvalue emit_increment_expression(encoder *const enc, const node *const nd)
{
	...
	if (is_prefix)
	{
		to_code_2R_I(enc->sx->io, IC_MIPS_ADDI, operand_rvalue.val.reg_num, operand_rvalue.val.reg_num, imm);
	...
	else
	{
		...
		// FIXME: надо в определенный rvalue это засовывать, добавить это в emit_binary_operation!
...
static void emit_array_declaration(encoder *const enc, const node *const nd)
{
	...
	// Сохранение регистров аргументов (чтобы они не были затёры). Но вообще-то это надо бы переделать.
...
static void emit_switch_statement(encoder *const enc, const node *const nd)
{
	...
	assert(mips_type_is_integer(enc->sx, expression_get_type(&condition))); // Согласно стандарту Си

	size_t label_num = enc->label_num; // Увеличиваться enc->label_num будет далее
	...
	for (size_t i = 0; i < amount; i++)
	{
		...
		{
			const size_t case_num = label_num++;
			...
			// TODO: заменить на новую версию
			const rvalue result_rvalue = emit_binary_operation(enc, &condition_rvalue, &case_expr_rvalue, BIN_EQ);

			emit_conditional_branch(enc, IC_MIPS_BEQ, &result_rvalue, &label_case);
		}
		else if (substmt_class == STMT_DEFAULT)
		{
			const size_t default_num = label_num++;
