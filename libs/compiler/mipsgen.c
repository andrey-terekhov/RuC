 ...
 */
static void emit_conditional_branch(encoder *const enc, const mips_instruction_t instruction, const rvalue *const value,
    ...
    if (target->kind == LVALUE_KIND_REGISTER)
    {
        if (value->val.reg_num != target->loc.reg_num)
        {
            const mips_instruction_t instruction =
                type_is_floating(enc->sx, value->type) ? IC_MIPS_MOV_S : IC_MIPS_MOVE;
            uni_printf(enc->sx->io, "\t");
            instruction_to_io(enc->sx->io, instruction);
            uni_printf(enc->sx->io, " ");
...
static rvalue emit_printf_expression(encoder *const enc, const node *const nd)
{
    ...
    for (size_t i = 1; i < parameters_amount; i++)
    {
        ...
        to_code_2R_I(enc->sx->io, IC_MIPS_ADDI, R_SP, R_SP,
                     -(item_t)WORD_LENGTH *
    ...
    if (func_ref >= BEGIN_USER_FUNC)
    {
        ...
        for (size_t i = 0; i < params_amount; i++)
        {
            ...
            if ((type_is_floating(enc->sx, arg_rvalue.type) ? f_arg_count : arg_count) < ARG_REG_AMOUNT)
            {
                ...
                mips_register_to_io(
                    enc->sx->io, (type_is_floating(enc->sx, arg_rvalue.type) ? R_FA0 + f_arg_count : R_A0 + arg_count));
            ...
            const rvalue arg_saved_rvalue = {
                .kind = RVALUE_KIND_REGISTER,
                .val.reg_num = (type_is_floating(enc->sx, arg_rvalue.type) ? R_FA0 + f_arg_count : R_A0 + arg_count),
                .type = arg_rvalue.type,
                .from_lvalue = !FROM_LVALUE
            };
            ...
            if ((type_is_floating(enc->sx, arg_rvalue.type) ? f_arg_count : arg_count) < ARG_REG_AMOUNT)
            {
                ...
                emit_move_rvalue_to_register(
                    enc, type_is_floating(enc->sx, arg_rvalue.type) ? (R_FA0 + f_arg_count) : (R_A0 + arg_count),
                    &arg_rvalue);
        ...
        while (i + j < arg_reg_count)
        {
            ...
            emit_move_rvalue_to_register(
                enc, type_is_floating(enc->sx, prev_arg_displ[i + j].type) ? (R_FA0 + 2 * j++) : (R_A0 + i++),
                &tmp_rval);
...
static rvalue emit_unary_expression(encoder *const enc, const node *const nd)
{
    ...
    switch (operator)
    {
        ...
        case UN_ABS:
        {
            ...
            const rvalue operand_rvalue = emit_expression(enc, &operand);
            const mips_instruction_t instruction =
                type_is_floating(enc->sx, operand_rvalue.type) ? IC_MIPS_ABS_S : IC_MIPS_ABS;
        ...
        case UN_ADDRESS:
        {
            ...
            to_code_2R_I(enc->sx->io, IC_MIPS_ADDI, result_rvalue.val.reg_num, operand_lvalue.base_reg,
                            ...
                            const rvalue *const addr)
{
    ...
    for (size_t i = 0; i < amount; i++)
    {
        ...
        else
        {
            ...
            if (i != amount - 1)
            {
                to_code_2R_I(enc->sx->io, IC_MIPS_ADDI, addr->val.reg_num, addr->val.reg_num, -4);
            }
...
static void emit_function_definition(encoder *const enc, const node *const nd)
{
    ...
    {
        to_code_R_I_R(
            enc->sx->io, IC_MIPS_S_S, R_FS0 + 2 * i,
