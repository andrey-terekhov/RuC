...
typedef struct information
{
	...
	vector arrays; 						/**< Вектор, в котором хранятся смещения в стеке, в которых записаны размеры
					   						выделенных в куче массивов */
} information;
...
static void to_code_3R(universal_io *const io, const mips_instruction_t instruction, const mips_register_t fst_reg,
					   const mips_register_t snd_reg, const mips_register_t thd_reg)
{
	uni_printf(io, "\t");
	instruction_to_io(io, instruction);
	uni_printf(io, " ");
...
static void to_code_R_FPI(universal_io *const io, const mips_instruction_t instruction, const mips_register_t reg,
						  const double imm)
{
	uni_printf(io, "\t");
	instruction_to_io(io, instruction);
...
static void to_code_L(universal_io *const io, const mips_instruction_t instruction, const mips_label_t label,
...
static void to_code_R_L(universal_io *const io, const mips_instruction_t instruction, const mips_register_t reg,
...
static void to_code_2R_L(universal_io *const io, const mips_instruction_t instruction, const mips_register_t fst_reg,
						 const mips_register_t snd_reg, const mips_label_t label, const item_t label_num)
{
	uni_printf(io, "\t");
	instruction_to_io(io, instruction);
	uni_printf(io, " ");
...
static void store_lvalue(information *info, const node *const nd, lvalue nd_lvalue, const mips_register_t reg_to_store)
{
	...
	{
		to_code_R_I_R(info->sx->io, (type_is_floating(nd_lvalue.type)) ? IC_MIPS_S_S : IC_MIPS_SW, reg_to_store, 
					  R_ZERO, nd_lvalue.loc);
	}
	else
	{
		to_code_R_I_R(info->sx->io, (type_is_floating(nd_lvalue.type)) ? IC_MIPS_S_S : IC_MIPS_SW, reg_to_store,
...
static void load_lvalue(information *info, const node *const nd, lvalue nd_lvalue, const mips_register_t reg_to_load)
{
	if (expression_get_class(nd) == EXPR_SUBSCRIPT)
	{
		to_code_R_I_R(info->sx->io, (type_is_floating(nd_lvalue.type)) ? IC_MIPS_L_S : IC_MIPS_LW, reg_to_load, 
					  R_ZERO, nd_lvalue.loc);
	}
	else
	{
...
static mips_register_t get_subs_displ_on_reg(information *info, const node *const nd)
{
	...
	// TODO: придумать что-нибудь с base_addr_reg

	return index_rvalue_reg;
}
		...
		for (size_t i = 0; i < args_amount; i++)
		{
			...
			if (type_is_floating(arg_type))
			{
				if (f_arg_count < ARG_REG_AMOUNT) // в регистр fa0-fa3
				{
					...
					{
						to_code_R_I(info->sx->io, IC_MIPS_LI_S, /* код регистра fa0-fa3 -> */ 36 + f_arg_count,
									arg_rvalue.val.float_val);
					}
					else
					{
						to_code_R_I_R(info->sx->io, IC_MIPS_L_S, /* код регистра fa0-fa3 -> */ 36 + f_arg_count,
			...
			else
			{
				if (arg_count < 4) // в регистр a0-a3
				{
					if ((type_is_scalar(info->sx, arg_type)) || (type_is_array(info->sx, arg_type)))
					{
						...
						if (arg_rvalue.kind == CONST)
						{
							to_code_R_I(info->sx->io, IC_MIPS_LI, /* код регистра a0-a3 -> */ 4 + arg_count,
										arg_rvalue.val.int_val);
						}
						else
						{
...
static rvalue emit_logic_expression(information *const info, const node *const nd)
{
	...
	{
		to_code_2R_L(info->sx->io, get_instruction(info, operation, /* Два регистра => 0 в get_instruction() -> */ 0),
					 result, R_ZERO, L_ELSE, info->label_else);
	}
	else
	{
		to_code_R_L(info->sx->io, get_instruction(info, operation, /* Два регистра => 0 в get_instruction() -> */ 0),
					result, L_ELSE, info->label_else);
	}
...
static rvalue emit_integral_expression(information *const info, const node *const nd)
{
	...
	to_code_3R(info->sx->io, get_instruction(info, operator, /* Два регистра => 0 в get_instruction() -> */ 0), result,
			   left_reg, right_reg);
...
static rvalue emit_assignment_expression(information *const info, const node *const nd)
{
	...
	if (operator!= BIN_ASSIGN) // это "+=", "-=" и т.п.
	{
		...
		to_code_3R(info->sx->io,
				   get_instruction(info, operator, /* два регистра в операции => ноль в get_instruction() -> */ 0),
...
static lvalue emit_lvalue(information *info, const node *const nd)
{
	switch (expression_get_class(nd))
	{
		...
		case EXPR_SUBSCRIPT:
			// Для вырезки из массива в .loc у emit_subscript_expression() будет лежать регистр, в который записан адрес
			// элемента массива
		...
		*/
		case EXPR_LITERAL:
			...
			case EXPR_SUBSCRIPT:
			{
				...
				// Грузим на регистр
				to_code_R_I_R(info->sx->io, (type_is_floating(lv.type)) ? IC_MIPS_L_S : IC_MIPS_LW, tmp_reg, R_ZERO,
			lv.loc);
...
static void emit_variable_declaration(information *const info, const node *const nd)
{
	...
	*/
	...
	{
		// TODO: Оптимизация: для статических массивов выделять в куче место не нужно
		...
		vector_add(&info->arrays, -(item_t)value_displ);
		...
		if (has_init)
		{
			...
			const mips_register_t elem_count_reg = get_register(info);
			...
			for (size_t j = 1; j <= dim; j++)
			{
				...
				const size_t init_size = expression_initializer_get_size(&init);
				
				for (size_t j = 0; j < init_size; j++)
				{
					const node init_subexpr = expression_initializer_get_subexpr(&init, j);
					const rvalue init_rvalue = emit_expression(info, &init_subexpr);

					const mips_register_t init_rvalue_reg = get_rvalue_reg(info, init_rvalue); 
					...
					// TODO: нет ERROR метки в кодогенераторе, так что приходится пока так
					uni_printf(info->sx->io, "\tbgtz ");
					mips_register_to_io(info->sx->io, buf_reg);
					uni_printf(info->sx->io, ", ERROR\n");

					// Загрузка текущего элемента массива -- в $v0 лежит его адрес
					uni_printf(info->sx->io, "\n\t# loading value:\n");
					to_code_R_I_R(info->sx->io, IC_MIPS_SW, init_rvalue_reg, 0, R_V0);

					// Сдвигаем адрес и увеличиваем счётчик числа элементов
					uni_printf(info->sx->io, "\n\t# incrementing element counter (");
					mips_register_to_io(info->sx->io, elem_count_reg);
					uni_printf(info->sx->io, ") and address ($v0):\n");
					...
					while (get_register_amount(info) > curr_reg + 1) // +1, чтобы не освободить tmp_reg
					{
						free_register(info);
					}
				}
...
static void emit_function_definition(information *const info, const node *const nd)
{
	...
	if (!is_main)
	{
		...
		{
			to_code_R_I_R(info->sx->io, IC_MIPS_SW, /* код регистра -> */ 16 + i,
						  /* 4 за ra (sp уже учтен) + смещение -> */ 4 + 4 * (i + 1), R_SP);
		}
		...
		{
			to_code_R_I_R(info->sx->io, IC_MIPS_S_S, /* код регистра -> */ 52 + 2 * i,
						  /* 36 за ra + s0-s7 + смещение -> */ 36 + 4 * (i + 1), R_SP);
		}
	...
	for (size_t i = 0; i < 8; i++)
	{
		to_code_R_I_R(info->sx->io, IC_MIPS_LW, /* код регистра -> */ 16 + i,
					  /* 4 за ra (sp уже учтен) + смещение -> */ 4 + 4 * (i + 1), R_SP);
	}
	...
	for (size_t i = 0; i < 5; i++)
	{
		to_code_R_I_R(info->sx->io, IC_MIPS_L_S, /* код регистра -> */ 52 + 2 * i,
					  /* 36 за ra + s0-s7 + смещение -> */ 36 + 4 * (i + 1), R_SP);
	}

	uni_printf(info->sx->io, "\n");

	// Восстановление размеров кучи
	uni_printf(info->sx->io, "\t# heap deallocation:\n");

	to_code_R_I(info->sx->io, IC_MIPS_LI, R_A0, 0);

	mips_register_t tmp_reg = get_register(info);
	uni_printf(info->sx->io, "\t# loading array sizes into ");
	mips_register_to_io(info->sx->io, tmp_reg);
	uni_printf(info->sx->io, ":\n");

	for (size_t i = 0; i < vector_size(&info->arrays); i++)
	{
		// Загружаем в свободный регистр размеры массивов, выделенных в ходе работы функции
		to_code_R_I_R(info->sx->io, IC_MIPS_LW, tmp_reg, vector_get(&info->arrays, i), R_SP);

		to_code_3R(info->sx->io, IC_MIPS_ADD, R_A0, R_A0, tmp_reg);
	}
	// В $a0 таким образом окажутся размеры всех массивов -- умножим их на (-1), 
	// чтобы далее уменьшить кучу
	const mips_register_t neg_one_reg = get_register(info);
	to_code_R_I(info->sx->io, IC_MIPS_LI, neg_one_reg, -1);
	to_code_3R(info->sx->io, IC_MIPS_MUL, R_A0, R_A0, neg_one_reg);
	free_register(info);

	uni_printf(info->sx->io, "\t# $a0 now holds the negative sum of all array sizes\n\n");

	// Системный вызов по обращению к куче
	to_code_R_I(info->sx->io, IC_MIPS_LI, R_V0, 9);
	uni_printf(info->sx->io, "\tsyscall\n"); 
...
static void pregen(syntax *const sx)
{
	...
	uni_printf(sx->io, "\t.align 2\n"); // выравнивание последующих данных/команд по границе, кратной 2^n байт (в данном
										// случае 2^2 = 4)

	uni_printf(sx->io, "\n\t.globl\tmain\n"); // делает метку main глобальной -- её можно вызывать извне кода (например,
...
static void postgen(information *const info)
{
	...
	to_code_R_I_R(info->sx->io, IC_MIPS_LW, R_RA, -4,
				  R_SP); // загружаем в return address значение из -4($sp) (что там находится?)
