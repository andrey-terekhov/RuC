...
item_t parse_array_definition(parser *const prs, node *const parent, item_t type)
{
	...
	while (token_try_consume(prs, TOK_LSQUARE))
	{
		...
		if (token_try_consume(prs, TOK_RSQUARE))
		{
			if (prs->token == TOK_LSQUARE)
			{
		...
		else
		{
			...
			if (!token_try_consume(prs, TOK_RSQUARE))
			{
				...
				token_skip_until(prs, TOK_RSQUARE | TOK_COMMA | TOK_SEMICOLON);
			}
		}
...
item_t parse_struct_declaration_list(parser *const prs, node *const parent)
{
	...
	do
	{
		...
		if (token_try_consume(prs, TOK_STAR))
		{
		...
		if (token_try_consume(prs, TOK_IDENTIFIER))
		{
			if (prs->token == TOK_LSQUARE)
			{
				...
				if (token_try_consume(prs, TOK_EQUAL))
				{
		...
		else
		{
			...
			token_skip_until(prs, TOK_SEMICOLON | TOK_RBRACE);
		}
		...
		token_expect_and_consume(prs, TOK_SEMICOLON, no_semicolon_in_struct);
	} while (!token_try_consume(prs, TOK_RBRACE));
...
{
	if (!token_try_consume(prs, TOK_LBRACE))
	{
		parser_error(prs, struct_init_must_start_from_BEGIN);
		token_skip_until(prs, TOK_COMMA | TOK_SEMICOLON);
		return;
	}
	...
	do
	{
		...
		if (prs->token == TOK_RBRACE)
		{
			break;
		}
		else if (!token_try_consume(prs, TOK_COMMA))
		{
...
void parse_init_declarator(parser *const prs, node *const parent, item_t type)
{
	...
	if (prs->token == TOK_LSQUARE)
	{
		...
		if (!prs->flag_empty_bounds && prs->token != TOK_EQUAL)
		{
...
item_t parse_function_declarator(parser *const prs, const int level, int func_def, const item_t return_type)
{
	...
	else
	{
		do
		{
			...
			else if (prs->token == TOK_IDENTIFIER)
			{
			...
			if (type == mode_void && prs->token != TOK_LPAREN)
			{
			...
			if (prs->token == TOK_LSQUARE)
			{
				...
				while (token_try_consume(prs, TOK_LSQUARE))
				{
					...
					{
						parser_error(prs, wait_right_sq_br);
						token_skip_until(prs, TOK_RSQUARE | TOK_COMMA | TOK_RPAREN | TOK_SEMICOLON);
					}
				}
			}
			...
			{
				token_expect_and_consume(prs, TOK_STAR, wrong_func_as_arg);
				...
				token_expect_and_consume(prs, TOK_RPAREN, no_right_br_in_arg_func);
				token_expect_and_consume(prs, TOK_LPAREN, wrong_func_as_arg);
				if (arg_func == 1)
				{
		...
		token_expect_and_consume(prs, TOK_RPAREN, wrong_param_list);
...
void parse_function_definition(parser *const prs, node *const parent, const item_t type)
{
	...
	if (prs->func_def == 0 && prs->token == TOK_LBRACE)
	{
		prs->func_def = 1;
	}
	...
	else if (prs->func_def == 1)
	{
		...
		token_try_consume(prs, TOK_SEMICOLON);
	}
}
...
void parse_declaration_inner(parser *const prs, node *const parent)
{
	...
	else if (prs->flag_was_type_def && token_try_consume(prs, TOK_SEMICOLON))
	{
		return;
	}
	...
	token_expect_and_consume(prs, TOK_SEMICOLON, expected_semi_after_decl);
}
...
void parse_declaration_external(parser *const prs, node *const root)
{
	...
	if (prs->flag_was_type_def && token_try_consume(prs, TOK_SEMICOLON))
	{
		return;
	}
