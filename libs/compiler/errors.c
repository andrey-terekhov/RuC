...
static void get_error(const err_t num, char *const msg, va_list args)
{
    switch (num)
    {
        ...
        case struct_without_init:
            sprintf(
                msg,
                "встречено объявление структуры, нуждающейся в инициализации (с константным полем), без инициализации");
            break;
        case array_without_init:
            sprintf(msg, "встречено объявление массива с элементами, нуждающимися в инициализации (являющимися "
                         "константным полем), без инициализации");
            break;
...
static void output(universal_io *const io, const char *const msg, const logger system_func,
                   void (*func)(location *const, const char *const))
{
