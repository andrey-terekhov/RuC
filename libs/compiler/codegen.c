...
static int enc_export(const encoder *const enc)
{
    ...
    uni_printf(enc->sx->io, "%zi %zi %zi %zi %zi %" PRIitem " 0\n", vector_size(&enc->memory),
               vector_size(&enc->functions), vector_size(&enc->identifiers), vector_size(&enc->representations),
               vector_size(&enc->sx->types), enc->max_global_displ);

    return print_table(enc, &enc->memory) || print_table(enc, &enc->functions) || print_table(enc, &enc->identifiers) ||
           print_table(enc, &enc->representations) || print_table(enc, &enc->sx->types);
}
...
static lvalue emit_identifier_lvalue(encoder *const enc, const node *const nd)
{
    ...
    const item_t type = ident_get_type(enc->sx, identifier);
    const item_t unqualified_type =
        type_is_const(enc->sx, type) ? type_const_get_unqualified_type(enc->sx, type) : type;
...
static void emit_print_expression(encoder *const enc, const node *const nd)
{
    ...
    for (size_t i = 0; i < argc; i++)
    {
        ...
        const item_t unqualified_arg_type =
            type_is_const(enc->sx, arg_type) ? type_const_get_unqualified_type(enc->sx, arg_type) : arg_type;
...
static void emit_void_expression(encoder *const enc, const node *const nd)
{
    ...
    else
    {
        ...
        const size_t index =
            mem_get(enc, mem_size(enc) - 1) < MIN_INSTRUCTION_CODE ? mem_size(enc) - 2 : mem_size(enc) - 1;
