...
static inline size_t item_get_size(const item_status status)
{
    ...
    size_t size = status == item_int64 || status == item_uint64   ? 1
                  : status == item_int32 || status == item_uint32 ? 2
                  : status == item_int16 || status == item_uint16 ? 4
                                                                  : 8;
...
size_t item_store_int64_for_target(const item_status status, const int64_t value, item_t *const stg)
{
    ...
    const uint64_t sign = status == item_int64 || status == item_int32 || status == item_int16 || status == item_int8
                              ? (~mask >> 1) & mask
                              : 0;
