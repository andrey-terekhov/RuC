...
static inline char char_to_cp866(const char32_t symbol)
{
...
static inline char char_to_cp1251(const char32_t symbol)
{
	if (symbol >= U'А' && symbol <= U'Я')
	{
...
static size_t utf8_to_codepage(const char *const src, char *const dest, char (*char_to_codepage)(const char32_t))
{
