...
static bool mark_recognize(universal_io *const io, size_t *const line, size_t *const path, size_t *const code)
{
...
static bool mark_reverse_recognize(universal_io *const io, size_t *const line, size_t *const path, size_t *const code,
                                   size_t *const filler)
{
    size_t position = in_get_position(io);
    if (position == 0)
...
int loc_update(location *const loc)
{
    ...
    uni_printf(loc->io, "%s%c%zu%c%c%s%c\n", PREFIX, SEPARATOR, loc->line, SEPARATOR, QUOTE, path, QUOTE);
...
int loc_update_begin(location *const loc)
{
    ...
    uni_printf(loc->io, "%s%c%zu%c%c%s%c%c%s%c", PREFIX, SEPARATOR, loc->line, SEPARATOR, QUOTE, path, QUOTE, SEPARATOR,
               COMMENT, SEPARATOR);
...
int loc_search_from(location *const loc)
{
    ...
    while (begin != 0 && begin != loc->code)
    {
        ...
        {
            loc->line = line + diff;
            while (!mark_reverse_recognize(loc->io, &line, &path, &comment, &filler) && in_get_position(loc->io) != 0)
            {
            }
