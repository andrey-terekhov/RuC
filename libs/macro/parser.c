...
static inline char32_t skip_macro(parser *const prs, const keyword_t keyword)
{
    ...
    if ((keyword == KW_MACRO || keyword == KW_DEFINE || keyword == KW_SET || keyword == KW_UNDEF ||
         keyword == KW_IFDEF || keyword == KW_IFNDEF) &&
        !parse_name(prs))
    {
        out_clear(prs->io);
        return skip_directive(prs);
    ...
    while (character != '\n' && character != (char32_t)EOF)
    {
        ...
        else if (character == '#' && keyword != KW_DEFINE && keyword != KW_SET && keyword != KW_IF &&
                 keyword != KW_ELIF && keyword != KW_WHILE && keyword != KW_EVAL)
        {
            out_clear(prs->io);
...
static keyword_t skip_block(parser *const prs, const keyword_t begin)
{
    ...
    while (character != (char32_t)EOF)
    {
        ...
        if (keyword == KW_ELIF || keyword == KW_ELSE || keyword == KW_ENDIF || keyword == KW_ENDW || keyword == KW_ENDM)
        {
 ...
 */
static inline void parse_values(parser *const prs, const size_t index, storage *const stg, char *const value,
                                const size_t arg)
{
...
static bool parse_replacement(parser *const prs, const size_t index)
{
    ...
    if (expected == 0)
    {
        if (skip_lines(prs) != '(' || uni_scan_char(prs->io) != '(' || skip_lines(prs) != ')' ||
            uni_scan_char(prs->io) != ')')
        {
            *prs->loc = loc;
    ...
    {
        loc_search_from(prs->loc);
        parser_error(prs, prs->loc, expected > actual ? ARGS_REQUIRES : ARGS_PASSED, storage_to_string(prs->stg, index),
                     expected, actual);
    }
...
static inline void parse_path(parser *const prs, const char32_t quote)
{
    ...
    size_t index = quote == '"' ? linker_search_internal(prs->lk, path) : linker_search_external(prs->lk, path);
    free(path);
...
static inline bool parse_suffix(parser *const prs)
{
    ...
    switch (suffix[0])
    {
        ...
        case 'u':
            return size == 1 ||
                   ((suffix[1] == 'L' || suffix[1] == 'l') && (size == 2 || suffix[2] == 'L' || suffix[2] == 'l'));

        case 'L':
        case 'l':
            return size == 1 ||
                   ((suffix[1] == 'L' || suffix[1] == 'l') && (size == 2 || suffix[2] == 'U' || suffix[2] == 'u')) ||
...
static inline bool parse_token(parser *const prs, computer *const comp, const size_t pos)
{
    ...
    switch (character)
    {
        ...
        case '!':
            return computer_push_token(comp, pos, next == '=' && uni_scan_char(prs->io) == '=' ? TK_NOT_EQ : TK_NOT) ==
                   0;
        case '=':
            if (next != '=')
            {
        ...
        case '&':
            ...
            return computer_push_token(comp, pos, next == '&' && uni_scan_char(prs->io) == '&' ? TK_AND : TK_BIT_AND) ==
                   0;
        case '|':
            if (next == '=')
            {
            ...
            return computer_push_token(comp, pos, next == '|' && uni_scan_char(prs->io) == '|' ? TK_OR : TK_BIT_OR) ==
                   0;
        ...
        case '<':
            ...
            return computer_push_token(comp, pos,
                                       next == '=' && uni_scan_char(prs->io) == '=' ? TK_LESS_EQ : TK_LESS) == 0;
        case '>':
            if (next == '>')
            {
                uni_scan_char(prs->io);
            ...
            return computer_push_token(comp, pos,
                                       next == '=' && uni_scan_char(prs->io) == '=' ? TK_GREATER_EQ : TK_GREATER) == 0;
...
static item_t parse_expression(parser *const prs)
{
    ...
    computer comp =
        prs->prev == NULL ? computer_create(&loc, prs->io, directive) : computer_create(prs->prev, NULL, directive);
    size_t position = in_get_position(prs->io);
    universal_io out = io_create();
    char *buffer = NULL;
...
static bool parse_next(parser *const prs, const keyword_t begin, const keyword_t next)
{
    if (((begin == KW_IFDEF || begin == KW_IFNDEF || begin == KW_IF || begin == KW_ELIF) &&
        ...
        (begin == KW_MACRO && next == KW_ENDM))
    {
        return true;
    }

    parse_location(prs);
    parser_error(prs, prs->loc,
                 begin == KW_ELSE && (next == KW_ELIF || next == KW_ELSE) ? DIRECTIVE_AFTER : DIRECTIVE_WITHOUT,
